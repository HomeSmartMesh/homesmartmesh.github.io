...
async function get_section_data(section_path,href_base){
  ...
  const cache_base = join(config.rootdir,'cache',section_path)
  console.log(`menu> making path = ${cache_base}`)
  ...
  const section_menu_tree_file = join(cache_base,"section_menu_tree.json")
  const section_menu_list_file = join(cache_base,"section_menu_list.json")
  const section_list_file = join(cache_base,"section_files.json")
...
// '/content/blog', '/blog/gallery' => 'D:\Dev\MicroWebStacks\astro-big-doc\data\blog\gallery.mdx'
